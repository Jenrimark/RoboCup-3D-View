...
public:
	...
	Status setAEPEnabled(bool enabled)
	{
		return setProperty(STREAM_PROPERTY_AUTO_EXPOSURE_PRIORITY, enabled ? TRUE : FALSE);
	}
	...
	bool getAEPEnabled() const
	{
		OniBool enabled = FALSE;

		Status rc = getProperty(STREAM_PROPERTY_AUTO_EXPOSURE_PRIORITY, &enabled);
		return rc == STATUS_OK && enabled == TRUE;
	}
	...
	Status setBrightness(int brightness)
	{
		return setProperty(STREAM_PROPERTY_BRIGHTNESS, brightness);
	}

	int getSharpness()
	{
		int sharpness;
		Status rc = getProperty(STREAM_PROPERTY_SHARPNESS, &sharpness);
		if (rc != STATUS_OK)
		{
			return 0;
		}
	...
	Status setSharpness(int sharpness)
	{
		return setProperty(STREAM_PROPERTY_SHARPNESS, sharpness);
	}

	int getBrightness()
	{
		int brightness;
		Status rc = getProperty(STREAM_PROPERTY_BRIGHTNESS, &brightness);
		if (rc != STATUS_OK)
		{
			return 0;
		}
	...
	Status setSaturation(int saturation)
	{
		return setProperty(STREAM_PROPERTY_SATURATION, saturation);
	}

	int getSaturation()
	{
		int saturation;
		Status rc = getProperty(STREAM_PROPERTY_SATURATION, &saturation);
		if (rc != STATUS_OK)
		{
			return 0;
		}
	...
	Status setContrast(int contrast)
	{
		return setProperty(STREAM_PROPERTY_CONTRAST, contrast);
	}

	int getContrast()
	{
		int contrast;
		Status rc = getProperty(STREAM_PROPERTY_CONTRAST, &contrast);
		if (rc != STATUS_OK)
		{
			return 0;
		}
	...
	Status setGamma(int gamma)
	{
		return setProperty(STREAM_PROPERTY_GAMMA, gamma);
	}

	int getGamma()
	{
		int gamma;
		Status rc = getProperty(STREAM_PROPERTY_GAMMA, &gamma);
		if (rc != STATUS_OK)
		{
			return 0;
		}
	...
	Status setHue(int hue)
	{
		return setProperty(STREAM_PROPERTY_HUE, hue);
	}

	int getHue()
	{
		int hue;
		Status rc = getProperty(STREAM_PROPERTY_HUE, &hue);
		if (rc != STATUS_OK)
		{
			return 0;
		}
